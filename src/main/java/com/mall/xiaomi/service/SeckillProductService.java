...
public class SeckillProductService {
    ...
    public void deleteSeckillProduct(String seckillId) {
        SeckillProduct seckillProduct = new SeckillProduct();
        seckillProduct.setProductId(Integer.parseInt(seckillId));
        try {
            int count = seckillProductMapper.delete(seckillProduct);
            if (count != 1) {
                throw new XmException(ExceptionEnum.DELETE_SECKILLPRODUCT_ERROR);
            }
        } catch (XmException e) {
            e.printStackTrace();
            throw new XmException(ExceptionEnum.DELETE_SECKILLPRODUCT_ERROR);
    ...
    private void mqSend(String seckillId, Integer userId) {
        ...
        try {
            rabbitTemplate.convertAndSend("seckill_order", map, message -> {
                message.getMessageProperties().setCorrelationId(seckillId + ":" + userId);
                return message;
            });
